...
class database{
	...
	public function insert_to_db($table_name,$values){
		...
		switch ($table_name){
			...
			case 'class_teacher':
				...
				break;
			case 'team':
				$this->insert_data_to_team($name,$admin_id,$class_id,$stat,$status);
				break;
			case 'data':
	...
	private function insert_data_to_team($name,$admin_id,$class_id,$stat,$status){
		$this->connect->exec("INSERT INTO team(name,admin_id,class_id,stat) VALUES"
			. "('$name','$admin_id','$class_id','$stat','$status')");
	}
